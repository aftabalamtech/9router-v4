...
const OPTIONAL_FIELDS = [
  ...
  "consecutiveUseCount",
  "cached_jwt", "jwt_expires_at", "last_balance", "fail_count", "cooling_until", "cookie",
];
...
export async function reorderProviderConnections(providerId) {
  const db = await getAdapter();
  await db.transaction(() => reorderInTx(db, providerId));
}
...
export async function deleteProviderConnectionByEmailAndProvider(email, provider) {
  ...
  await db.transaction(async () => {
    ...
    for (const row of rows) {
      await db.run(`DELETE FROM providerConnections WHERE id = ?`, [row.id]);
      ok = true;
    }
    if (ok) {
