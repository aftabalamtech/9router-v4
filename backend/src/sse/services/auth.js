...
export async function getProviderCredentials(provider, excludeConnectionIds = null, model = null, options = {}) {
  ...
  try {
    ...
    // Filter out model-locked, excluded and capability-insufficient connections
    const availableConnections = connections.filter(c => {
      if (excludeSet.has(c.id)) return false;
      if (isModelLockActive(c, model)) return false;

      // Extract capability requirements from options (e.g. for video generations)
      const data = c.providerSpecificData || {};
      if (options.image_url && data.supportsImageRef === false) {
        log.info("AUTH", `${provider} | Filtered connection ${c.id?.slice(0, 8)}: does not support image reference`);
        return false;
      }
      if (options.video_url && data.supportsVideoRef === false) {
        log.info("AUTH", `${provider} | Filtered connection ${c.id?.slice(0, 8)}: does not support video reference`);
        return false;
      }
      if ((options.start_frame || options.end_frame) && data.supportsStartEndFrame === false) {
        log.info("AUTH", `${provider} | Filtered connection ${c.id?.slice(0, 8)}: does not support start/end frame`);
        return false;
      }
    ...
    const lockedCount = connections.filter(c => isModelLockActive(c, model) && !excludeSet.has(c.id)).length;
    if (excludeSet.size > 0 || lockedCount > 0) {
      log.debug("AUTH", `  → unavailable details: ${excludeSet.size} excluded, ${lockedCount} locked`);
    }
...
export function extractApiKey(request) {
  ...
  const xApiKey = request.headers?.["x-api-key"] ?? request.headers?.get?.("x-api-key");
  if (xApiKey) {
    return xApiKey;
  }
