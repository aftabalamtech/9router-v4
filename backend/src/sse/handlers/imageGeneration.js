...
export async function handleImageGeneration(request) {
  ...
  const preferredConnectionId = request.headers?.["x-connection-id"] ?? request.headers?.get?.("x-connection-id") ?? null;
  const wantsStream = (request.headers?.["accept"] ?? request.headers?.get?.("accept") ?? "").includes("text/event-stream");
  ...
  // Set fallback prompt for upscaling models (e.g., topaz-enhance) if prompt is missing or empty
  if (modelStr.includes("topaz-enhance") || modelStr.includes("upscale")) {
    if (!body.prompt || (typeof body.prompt === "string" && body.prompt.trim() === "")) {
      body.prompt = "Upscale and enhance image details";
    }
  }
...
async function handleSingleModelImage(body, modelStr, { wantsStream, binaryOutput, preferredConnectionId, apiKey } = {}) {
  ...
  if (NO_AUTH_PROVIDERS.has(provider)) {
    ...
    if (result.success) {
      ...
      saveUsageStats({ provider, model, tokens: usage, apiKey, label: "IMAGE USAGE" });
      saveRequestDetail(buildRequestDetail({
        ...
        response: { type: "image" },
        status: "success"
      }, { endpoint: "/v1/images/generations" })).catch(() => {});
  ...
  while (true) {
    ...
    const result = await handleImageGenerationCore({
      ...
      onRequestSuccess: async () => {
        ...
        const usage = { prompt_tokens: 0, completion_tokens: 1 };
        saveUsageStats({ provider, model, tokens: usage, connectionId: credentials.connectionId, apiKey, label: "IMAGE USAGE" });
        saveRequestDetail(buildRequestDetail({
