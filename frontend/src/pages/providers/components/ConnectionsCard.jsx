  ...
  return (
    <Modal isOpen={isOpen} title={`Add ${providerName || provider} API Key`} onClose={onClose}>
      <div className="flex flex-col gap-4">
        ...
        {isVideoProvider && (
          <div className="bg-sidebar/30 p-4 rounded-lg border border-accent/10 flex flex-col gap-3 my-2">
...
export default function ConnectionsCard({ providerId, isOAuth }) {
  ...
  const hasErrors = connections.some((c) => c.lastError || c.testStatus === "error");
          ...
          <div className="flex flex-wrap items-center gap-2">
            {connections.length > 0 && (
              <button
                ...
                title="Reset testStatus and lastError for all connections"
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-border text-xs text-text-muted hover:text-amber-500 hover:border-amber-500/40 hover:bg-amber-500/5 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <span className="material-symbols-outlined text-[14px]">{resettingAll ? "progress_activity" : "restart_alt"}</span>
                {resettingAll ? "Resetting..." : "Reset All Status"}
                {hasErrors && !resettingAll && (
                  <span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-red-500/15 text-red-500 text-[10px] font-bold">
                    !
                  </span>
                )}
              </button>
